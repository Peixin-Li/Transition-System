<?php
  ...
  define('TB_Q','questions');//问题表
   ...
   $s_id=$_SESSION['stuID'];
    ...
    //标准答案
	 $query_st="select * from questions where question_id='". $qid."'";
  ...
  function getQList($question_id=''){
		if($question_id != '') $where = array('question_id'=>$question_id);
		return fetchRows(TB_Q, $where, '', '');
	}
    $anomaly_count = 0;
						...
						foreach($syn_words as $key=>$syn_word){
							...
							foreach($syn_word as $synw){
								if($synw != ''){
									...
									if($syn_pos !== false){
										$score += $sw['weight']*0.8;
										$synw_gb = iconv('utf-8','utf-8',$synw);	//
